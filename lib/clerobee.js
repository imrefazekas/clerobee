...
function createConfig(length){
	...
	config.randomScale = Math.pow( 10, config.randomLength );
...
ClerobeeProto.isDerived = function(masterID, slaveID) {
	...
	console.log( '>>>>', this.derive( config, masterID ), slaveRandom );
...
ClerobeeProto.generate = function( basis ) {
	...
	console.log( ts, pid, nid, rid, nnId );
...
ClerobeeProto.getRandom = function(config) {
	...
	var random = Math.floor( Math.random() * config.randomScale );
	return this.toBase36String( random, config.randomLength );
};
