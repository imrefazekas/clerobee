...
ClerobeeProto.derive = function(config, masterID ) {
	...
	return this.getRandom(config, masterRandom, masterNano );
};
...
ClerobeeProto.generate = function( basis ) {
	...
	var ts = this.getTimestamp(config),
		pid = this.getProcessId(config),
		nid = this.getNetworkId(config),
		rid = (masterID ? this.derive(config, masterID) : this.getRandom(config)),
		nnId = this.getNanoTime(config);
...
ClerobeeProto.getRandom = function(config, masterRandom, masterNano) {
	config = config || this.config;

	var random = '';
	for( var i = 0; i<config.randomLength; i+= RANDOM_MAX_LENGTH ){
		var length = Math.min( config.randomLength-i, RANDOM_MAX_LENGTH );

		if( masterRandom && masterNano ){
			var sliceR = masterRandom.substring(i, i+length);

			random += padHex( Number( parseInt(sliceR, 36) + parseInt(masterNano, 36) ).toString(36), length ).substr( 0, length );
		}
		else{
